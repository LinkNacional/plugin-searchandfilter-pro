...
class Search_Filter_Taxonomy_Object_Walker extends Walker_Category {
    ...
    function start_el( &$output, $taxonomy_term, $depth = 0, $args = [], $id = 0 ) {
        ...
        if ($args['hierarchical'] == 1) {
            if ($taxonomy_term->parent == 0) {
				...
				$this->depth_track = [$taxonomy_term_id]; //reset the chain
            } else {
        ...
        if ((intval($sf_hide_empty) != 1) || ($option_count != 0)) {
            ...
            //altered by Link Nacional (2021-05-06)
            $product_id = 25;
            $praga_id = -8;
            $value_count = null;
            $type = $taxonomy_term->taxonomy;
            if ($type == 'post_tag') {
                $the_query = new WP_Query([
                    'post_type' => 'page',
                    'tag_id' => $taxonomy_term->term_id,
                    'cat' => $product_id, $praga_id
                ]);
            } else {
                $the_query = new WP_Query([
                    'post_type' => 'page',
                    'cat' => $taxonomy_term->cat_ID
                ]);
            }
            $value_count = $the_query->found_posts;

            if ( !empty($show_count) ) {
                if ($show_count_format_sf == 'inline') {
                    $option->label .= '&nbsp;&nbsp;(' . number_format_i18n($value_count) . ')';
                } else {
                    if ($show_count_format_sf == 'html') {
                        $option->label .= '<span class="sf-count">(' . number_format_i18n($value_count) . ')</span>';
                    }
                }
            }
            //end altered by Link Nacional (2021-05-06)
    ...
    private function get_term_link_template($term, $term_names) {
        ...
        uasort($term_names, [$this, 'sortStringByLength']);
